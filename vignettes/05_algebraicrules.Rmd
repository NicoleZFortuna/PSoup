...
*Table 1: The types of building blocks used to generate the equation representing a node.*
...
*Table 2: An example of the diagramatic representation of different edges and their resulting mathematical representation.*
...
![](diagramRules/necStim.png){width=80%} | $B[t] = 1 \times f(A[t-1])$
![](diagramRules/d.png){width=80%} | $D[t] = \frac{2 \times A[t-1]}{1 + B[t-1]} \times f(C[t-1])*d$

As a supplement to the above building blocks, it is possible to combine upstream nodes in the case that their influence only occurs when in combination with each other. An example of this could be an enzyme needing to interact with a substrate to produce a product.

*Table 3: Additional operator used to modify how incoming nodes affect the node of interest. The following equation dictates how these incoming nodes are treated, the final value of which is treated as a single input for the respective builing block that the interaction belongs in.*

SBGN-AF syntax | Algorithmic rule
--- | -
![](diagramRules/AND.png){width=60%} | $AND[t]=^r\sqrt{\prod\limits_{i=1}^{r}Node_i[t]}$
...
The above example does not include necessary stimulants, AND, or any exogenous supplies.
