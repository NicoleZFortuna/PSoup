...
To see the simulation condition that was run we can call:
...
This function can also return a string description of the condition that was run:

```{r}
reportCondition(output$screen[[6]], returnName = T)[2]
```

Depending on the number of conditions that have been modified, the string name of the condition may be unwieldy. 
...
```{r, out.width="90%", dpi=300, fig.align = 'center'}
reportCondition(output$screen[[2]], returnName = T)[2]
