...
Note that the hormone objects each have an extension indicating in which compartment they are in the diagram. The above network object is also natively available in the PSoup package, and can therefore be called directly:
...
```{r}
# add additional rows filled with the baseline value one
genotypeDef[2:3, ] <- 1 # 2:4 is equivalent to c(2, 3)
...
genotypeDef
```
Another method is to specify the column that you want to change using $, and then just indicating the row index

```{r}
# specify mutant conditions in the additional rows
genotypeDef$RMS1_R[3] <- 0 # changing the value of the 3nd row, 'RMS1_R' column to 0
genotypeDef$RMS1_S[3] <- 0 # changing the value of the 3nd row, 'RMS1_S' column to 0
...
```{r, eval=FALSE}
# save modified genotypeDef object so that it can be used 
...
>
> In the case that you are uncomfortable interacting with a data.frame via R, you can export the data.frame as a csv object. Once you are happy with the conditions that you want to simulate, you must convert your csv object back into a data.frame, and save it as demonstrated above. **Keep in mind that you must not modify the column names!**
...
> In Excel, add a row of 1s for every condition that you want to test. Then modify values to corespond to the conditions that you want to test, with each row representing one 'experimental condition'. Leave the first row as 1s. Once you are happy with the simulations that you want to run, save your Excel file. You will need to load the updated Excel file in to R, so that you can save is as an R data.frame.
